...
class SL1Parser extends JavaTokenParsers {
  ...
  def cons: Parser[Expr] = (factor <~ "::") ~ (valOrFuncall | "Nil" | cons) ^^ {
    case expr ~ ("Nil") => Cons(expr, Variable("Nil"))
    case expr ~ (list: Expr) => Cons(expr, list)
  }

  def factor: Parser[Expr] = wholeNumber ^^ { x : String => Number(x.toInt) } | valOrFuncall
...
object SL1 extends App {
  ...
  val head: ListOp = ListOp(x => x.head)
  val tail: ListOp = ListOp(x => x.tail)
  val isEmpty: ListOp = ListOp(l => if (l.isEmpty) 1 else 0)
  parseResult match {
    case parser.Success(result: Block, next) => println(evalBlock(result, List(("Nil",List()),("head",head),("tail",tail),("isEmpty",isEmpty))))
